...
import { changePlan, createCheckout, getErrorMessage, getSubscription } from '@/lib/billingApi'
...
export function CheckoutPage() {
  ...
  return (
    <main className="relative min-h-dvh overflow-hidden bg-[#f6f0f7] text-[#201a25]">
      <div className="pointer-events-none absolute inset-0 opacity-70" aria-hidden="true" style={{ backgroundImage: 'radial-gradient(circle at 15% 15%, rgba(183,243,107,.32), transparent 24%), radial-gradient(circle at 88% 80%, rgba(79,55,138,.16), transparent 30%)' }} />
      ...
      <div className="relative mx-auto grid max-w-[1040px] gap-8 px-5 pb-14 pt-5 sm:px-8 lg:grid-cols-[minmax(0,1.08fr)_minmax(340px,.92fr)] lg:items-start lg:pt-12">
        <motion.section initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="rounded-[28px] border border-white/80 bg-white/90 p-6 shadow-[0_28px_80px_rgba(46,32,51,0.12)] backdrop-blur sm:p-9">
          ...
          <p className="mt-4 max-w-lg text-sm leading-6 text-[#6a6170]">Review your order here, then continue to Stripe’s encrypted payment page to enter your card and billing details.</p>

          <div className="mt-8 grid gap-3 sm:grid-cols-3">
            {['256-bit encrypted payment', 'Instant credit activation', 'Cancel whenever you need'].map((item) => (
          ...
          <div className="mt-8 rounded-2xl border border-[#dfd3e7] bg-[#f8f2f9] p-4">
            <div className="flex items-start gap-3"><LockKeyhole className="mt-0.5 size-5 shrink-0 text-[#4f378a]" /><div><p className="text-sm font-bold">Your card data never touches our servers</p><p className="mt-1 text-xs leading-5 text-[#746b79]">Stripe securely collects and processes the payment. AetherFlow only receives the subscription result.</p></div></div>
          </div>
        </motion.section>

        <motion.aside initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .08 }} className="overflow-hidden rounded-[28px] bg-[#2f185f] text-white shadow-[0_28px_80px_rgba(47,24,95,.25)]">
          <div className="border-b border-white/12 p-6 sm:p-7">
            ...
            <div className="mt-6 flex items-end gap-2"><span className="font-display text-5xl font-bold tracking-[-1px]">{formatPlanPrice(priceCents)}</span><span className="mb-1.5 text-sm text-white/55">/ {interval}</span></div>
          ...
          <div className="p-6 sm:p-7">
            ...
            <button type="button" onClick={handlePayment} disabled={loading || submitting || isExactCurrentPlan} className="group mt-7 flex min-h-14 w-full items-center justify-center gap-2 rounded-xl bg-[#d9ffa8] px-5 text-sm font-bold text-[#203707] shadow-[0_12px_30px_rgba(183,243,107,.18)] transition hover:bg-[#c9f887] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-[#2f185f] disabled:cursor-not-allowed disabled:opacity-60">
              {loading || submitting ? <Loader2 className="size-4 animate-spin" /> : <CreditCard className="size-4" />}
              {loading ? 'Checking subscription…' : submitting ? 'Opening secure payment…' : isExactCurrentPlan ? 'This is your current plan' : isActive ? 'Confirm plan change' : 'Continue to secure payment'}
